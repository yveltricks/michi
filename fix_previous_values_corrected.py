...
# Let's print the line numbers with the markers to help us find them
print("Finding the functions in the file...")
lines = content.splitlines()
for i, line in enumerate(lines):
    if "previous-values" in line:
        print(f"Line {i+1}: {line}")

# Search for the function by looking for line numbers
# Based on the output above, adjust these line numbers if needed
line_number_start = None
for i, line in enumerate(lines):
    if '<int:exercise_id>/<int:set_number>' in line:
        line_number_start = i
        break

if line_number_start is None:
    print("Could not find the second previous-values function!")
    exit(1)

# Find the next route after this one
line_number_end = None
for i in range(line_number_start + 1, len(lines)):
    if line.startswith('@workout.route'):
        line_number_end = i
        break

if line_number_end is None:
    print("Could not find the next function after the problematic one!")
    exit(1)

# Extract the content to replace
before_function = '\n'.join(lines[:line_number_start])
after_function = '\n'.join(lines[line_number_end:])
